...
#define WIN32_LEAN_AND_MEAN
#include <windows.h>
#include <imm.h>                  // 添加IME支持所需的头文件
#pragma comment(lib, "imm32.lib") // 添加imm32库

// 添加缺失的常量定义
#ifndef IMC_GETOPENSTATUS
#define IMC_GETOPENSTATUS 0x0001
#endif
...
static bool ImGui_ImplWin32_InitEx(void *hwnd, bool platform_has_own_dc)
{
    ...
    bd->hImc = ::ImmGetContext(bd->hWnd); // 初始化IME上下文
...
IMGUI_IMPL_API bool ImGui_ImplWin32_Init(void *hwnd)
{
    ...
    // 获取窗口的IME上下文
    if (hwnd)
        bd->hImc = ::ImmGetContext((HWND)hwnd);
...
void ImGui_ImplWin32_Shutdown()
{
    ...
    if (bd->hImc)
    {
        ::ImmReleaseContext(bd->MouseHwnd, bd->hImc);
        bd->hImc = NULL;
    }
...
IMGUI_IMPL_API LRESULT ImGui_ImplWin32_WndProcHandler(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_IME_SETCONTEXT:
    {
        ...
        if (ImGui::GetCurrentContext() && (HWND)lParam == hwnd)
        {
            // 如果ImGui想要捕获键盘输入，则激活IME
            BOOL fOpen = ((DWORD)wParam & IMC_GETOPENSTATUS) != 0;
            if (io.WantCaptureKeyboard && !fOpen)
                ::ImmSetOpenStatus(bd->hImc, TRUE);
        }
        return result;
    }
    case WM_IME_COMPOSITION:
    {
        if (ImGui::GetCurrentContext() && io.WantCaptureKeyboard)
        {
            ...
            if (lParam & GCS_RESULTSTR)
            {
                ...
                {
                    ::ImmGetCompositionStringW(hIMC, GCS_RESULTSTR, buffer, dwSize);
                    // 修复：使用循环调用正确的方法AddInputCharacterUTF16
                    for (int i = 0; i < dwSize / sizeof(WCHAR) && buffer[i] != 0; i++)
